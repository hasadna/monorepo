        ...
        try {
            CSVParser parser =
                    CSVParser.parse(
                            ...
                            CSVFormat.DEFAULT.withHeader("id","gtfs_stop_id","is_source","is_dest","actual_arrival","exp_arrival",
                                    "actual_departure","exp_departure","delay_arrival","delay_departure","index","filename","line_number","valid","invalid_reason",
                                    "stop_id","trip_id","sheet_idx","actual_departure_fixed","actual_arrival_fixed","ignored_error"));
        ...
        try {
            CSVParser parser =
                    CSVParser.parse(
                            stopCsv,
                            StandardCharsets.UTF_8,
                            CSVFormat.DEFAULT.withHeader("id","gtfs_stop_id","english","hebrews","lat","lon"));
            boolean isHeader = true;
            for (CSVRecord record : parser) {
                if (isHeader) {
                    isHeader = false;
    ...
    public static List<RailWayProtos.Trips> getTrips(Path tripsCsv) {
        ...
        try {
            ...
            for (CSVRecord record : parser) {
                ...
                tripsList.add(
                        tripBuilder.setId(record.get("id"))
